...
fn handle_connection(mut stream: TcpStream){
    ...
    let detection_os = stream.read(&mut buffer).unwrap();
    let detection_os = String::from_utf8_lossy(&buffer[..detection_os]);
    let detection_os = detection_os.trim_end();
        
    println!("New connection {addr} with os {detection_os}");
...
fn main(){
    ...
    let listener = TcpListener::bind("192.168.1.7:9999").unwrap();
